...
A Token By Degens For Degens ;D
...
contract RBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("RBNB_Dividend_Tracker", "RBNB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**9); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "RBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "RBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "RBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract RocketBNB is ERC20, Ownable {
    IPinkAntiBot public pinkAntiBot;
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    RBNBDividendTracker public dividendTracker;

    address public _marketingWallet = 0xbc2D2e5d59E16B7e2b02932c1F7FE8603C97B823;

    bool public restrictWhales = false;

    uint256 public maxTxAmount = 1000000000 * 10**6 * (10**9);
    uint256 public walletMax = 1000000000 * 10**6 * (10**9);
    uint256 public swapTokensAtAmount = 250000000000 * (10**9);
    ...
    constructor() ERC20("RocketBNB", "ROCKETBNB") {

        dividendTracker = new RBNBDividendTracker();
        BuyTotalFees = BuyRewardsFee.add(BuyLiquidityFee).add(BuyMarketingFee);
        SellTotalFees = SellRewardsFee.add(SellLiquidityFee).add(SellMarketingFee);
        ...
        dividendTracker.excludeFromDividends(0x0000000000000000000000000000000000000000);
        ...
        */
        _mint(owner(), 1000000000 * 10**6 * (10**9));  
    }

    receive() external payable {}

    function updateBuyFees(uint256 newBuyRewardFee, uint256 newBuyLiqFee, uint256 newBuyMarketingFee) public onlyOwner {
        BuyRewardsFee = newBuyRewardFee;
        BuyLiquidityFee = newBuyLiqFee;
        BuyMarketingFee = newBuyMarketingFee;
        BuyTotalFees = BuyRewardsFee.add(BuyLiquidityFee).add(BuyMarketingFee);
    }

    function updateSellFees(uint256 newSellRewardFee, uint256 newSellLiqFee, uint256 newSellMarketingFee) public onlyOwner {
        SellRewardsFee = newSellRewardFee;
    ...
    function changeIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            dividendTracker.excludeFromDividends(holder);
             ...
             try dividendTracker.includeInDividends(payable(holder), balanceOf(holder)) {} catch {}
        }
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RBNB: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RBNB: Cannot update gasForProcessing to same value");
    ...
    function prepareForLaunch() external onlyOwner {
        updateBuyFees(8,4,2);
        updateSellFees(10,3,2);
        restrictWhales = true;
        cooldownEnabled = false;
        maxTxAmount = 10000000 * 10**6 * (10**9);
        walletMax = 10000000 * 10**6 * (10**9);
    }
    ...
    function prepareForPresale() external onlyOwner {
        ...
        walletMax =  1000000000 * 10**6 * (10**9);
    }
